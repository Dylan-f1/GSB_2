...
namespace GSB_2.DAO
{
    public class AppartientDAO
    {
        ...
        // Ajouter un médicament à une prescription
        public bool addMedicineToPrescrition(int id_prescription, int id_medicine)
        {
            using (var connection = db.GetConnection())
            {
                try
                {
                    ...
                    myCommand.CommandText = @"INSERT INTO appartient (id_prescription, id_medicine)
                                            VALUES (@id_prescription, @id_medicine)";
        ...
        public List<Medicine> getMedicinesByPrescriptionId(int id_prescription)
        {
            ...
            using (var connection = db.GetConnection())
            {
                try
                {
                    ...
                    MySqlCommand myCommand = new MySqlCommand();
                    myCommand.Connection = connection;
                    myCommand.CommandText = @"SELECT m.id_medicine, m.id_users, m.name, m.description, m.molecule, m.dosage
                                            FROM medicine m
